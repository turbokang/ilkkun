...
# ilkkun PRD (Product Requirements Document)

---

## 1. Executive Summary

**ilkkun**(일꾼)은 여러 AI CLI 에이전트(Claude Code, Gemini CLI, Codex CLI)를 headless 모드로 실행하고, 스트리밍 JSON 출력을 Redis 큐에 순차적으로 푸시하는 경량 브릿지 CLI 도구이다.

단일 바이너리로 배포되어 별도 의존성 설치 없이 실행 가능하며, 다중 AI 에이전트 오케스트레이션 시스템의 기반 인프라로 활용된다.
...
- 다중 에이전트 오케스트레이션(ARISE 등)을 위해 이 출력을 중앙 큐로 수집 필요
...
| NG3 | 멀티턴 세션 관리 | 오케스트레이터(ARISE) 책임 |
...
Options:
  ...
  -p, --prompt <text>      프롬프트 [필수, stdin과 택일]
  -s, --session-id <id>    세션 ID (Redis 큐 키에 사용) [기본: uuid]
  -c, --cwd <path>         작업 디렉토리 [기본: 현재 디렉토리]
  -t, --timeout <seconds>  타임아웃 [기본: 300]
  --extra-args <args>      에이전트에 전달할 추가 인자
  --dry-run                실제 실행 없이 명령어 출력
  --no-yolo                자동 승인 비활성화 (기본: 활성화)
  -v, --version            버전 출력
  -h, --help               도움말 출력

Environment:
  REDIS_URL                Redis 연결 URL [기본: redis://localhost:6379]
  REDIS_QUEUE_PREFIX       큐 키 접두사 [기본: ilkkun:stream]
  REDIS_QUEUE_TTL          큐 TTL 초 [기본: 3600]
  ILKKUN_CLAUDE_BIN        Claude CLI 경로 [기본: claude]
  ILKKUN_GEMINI_BIN        Gemini CLI 경로 [기본: gemini]
  ILKKUN_CODEX_BIN         Codex CLI 경로 [기본: codex]
  ILKKUN_DEFAULT_AGENT     기본 에이전트 [기본: claude]
  ILKKUN_LOG_LEVEL         로그 레벨 (debug|info|warn|error) [기본: info]
...
| Docker | `ghcr.io/fermat/ilkkun:latest` |
| Homebrew | `brew install fermat/tap/ilkkun` (향후) |
      ...
      matrix:
        include:
          - os: ubuntu-latest
            ...
            artifact: ilkkun-linux-x64
          - os: ubuntu-latest
            target: bun-linux-arm64
            artifact: ilkkun-linux-arm64
          - os: macos-latest
            target: bun-darwin-x64
            artifact: ilkkun-darwin-x64
          - os: macos-latest
            target: bun-darwin-arm64
            artifact: ilkkun-darwin-arm64
          - os: windows-latest
            target: bun-windows-x64
            artifact: ilkkun-windows-x64.exe
...
| 주차 | 목표 | 산출물 |
|------|------|--------|
| 1 | 코어 구현 | CLI, Claude 어댑터, Redis 퍼블리셔 |
| 2 | 완성 | Gemini/Codex 어댑터, 테스트, 빌드 |
...
| 항목 | 우선순위 |
|------|----------|
| Redis Cluster/Sentinel | P2 |
| 메트릭 수집 (Prometheus) | P2 |
| 추가 에이전트 (Cursor 등) | P3 |
| 웹소켓 출력 모드 | P3 |
